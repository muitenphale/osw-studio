...
async function doSync(data: { sourceProjectId: string; paths: string[]; taskId: string }): Promise<void> {
  ...
  // Replay of server bytes: silent (no per-file DOM events) and dirty-suppressed so
  // noteContentEdit does not stamp updatedAt as if this tab edited.
  await saveManager.runWithSuppressedDirty(data.sourceProjectId, async () => {

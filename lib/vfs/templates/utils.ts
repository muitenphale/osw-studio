...
import { arrayBufferToBase64 } from '../binary-encoding';
import type { AssetConfig, ProjectTemplate } from './types';
...
/**
 * Adapt a saved custom template to the shape createProjectFromTemplate takes.
 *
 * A CustomTemplate stores binary file content as an ArrayBuffer while ProjectTemplate carries
 * strings, so binary has to be handed over base64-encoded with the isBase64 flag this function's
 * consumer already understands. Running it through TextDecoder instead — which is what both call
 * sites used to do — reinterprets the bytes as UTF-8 and destroys every image and font.
 */
export function customTemplateToProjectTemplate(template: {
  name: string;
  description: string;
  ...
  return {
    ...
    files: template.files.map((file) =>
      typeof file.content === 'string'
        ...
        : { path: file.path, content: arrayBufferToBase64(file.content), isBase64: true }
    ),

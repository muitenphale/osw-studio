 ...
 *
 * GET    /api/admin/sites/[id]/scheduled-functions/[scheduledFunctionId] - Get details
 * PUT    /api/admin/sites/[id]/scheduled-functions/[scheduledFunctionId] - Update
 * DELETE /api/admin/sites/[id]/scheduled-functions/[scheduledFunctionId] - Delete
 */
...
export async function GET(
  ...
  try {
    await requireAuth();
    const { id: siteId, scheduledFunctionId } = await params;
    ...
    const fn = siteDb.getScheduledFunction(scheduledFunctionId);
    if (!fn) {
...
export async function PUT(
  ...
  try {
    ...
    const existing = siteDb.getScheduledFunction(scheduledFunctionId);
    if (!existing) {
      return NextResponse.json({ error: 'Scheduled function not found' }, { status: 404 });
    }
    ...
    if (body.name !== undefined && body.name !== existing.name) {
      ...
      const duplicate = siteDb.getScheduledFunctionByName(body.name);
    ...
    if (body.functionId !== undefined) {
      const edgeFn = siteDb.getFunction(body.functionId);
      if (!edgeFn) {
    ...
    if (Object.keys(updates).length > 0) {
      siteDb.updateScheduledFunction(scheduledFunctionId, updates);
    }
...
export async function DELETE(
  ...
  try {
    ...
    siteDb.deleteScheduledFunction(scheduledFunctionId);

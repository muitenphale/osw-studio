 ...
 *
 * GET  /api/admin/sites/[id]/scheduled-functions - List all scheduled functions
 * POST /api/admin/sites/[id]/scheduled-functions - Create a new scheduled function
 */
...
/**
 * GET - List all scheduled functions for a site
 */
...
export async function POST(
  ...
  try {
    ...
    // Check limit (max 50 per site)
    const allScheduled = siteDb.listScheduledFunctions();
    if (allScheduled.length >= 50) {
      return NextResponse.json({ error: 'Maximum of 50 scheduled functions per site' }, { status: 400 });
    }

    // Check duplicate name
    const existing = siteDb.getScheduledFunctionByName(body.name);
    if (existing) {
    ...
    const id = siteDb.createScheduledFunction({
      name: body.name,
    ...
    const scheduledFunction = siteDb.getScheduledFunction(id);

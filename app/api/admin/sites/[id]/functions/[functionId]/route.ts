 ...
 *
 * GET    /api/admin/sites/[id]/functions/[functionId] - Get function details
 * PUT    /api/admin/sites/[id]/functions/[functionId] - Update function
 * DELETE /api/admin/sites/[id]/functions/[functionId] - Delete function
 */
...
export async function PUT(
  ...
  try {
    ...
    const existing = siteDb.getFunction(functionId);
    if (!existing) {
      return NextResponse.json({ error: 'Function not found' }, { status: 404 });
    }
    ...
    if (body.name !== undefined && body.name !== existing.name) {
      ...
      const duplicate = siteDb.getFunctionByName(body.name);
    ...
    if (Object.keys(updates).length > 0) {
      siteDb.updateFunction(functionId, updates);
    }
...
export async function DELETE(
  ...
  try {
    ...
    siteDb.deleteFunction(functionId);

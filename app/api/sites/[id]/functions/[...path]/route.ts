...
async function handleRequest(
  ...
  try {
    const { id: siteId, path } = await params;
    ...
    if (!site.databaseEnabled) {
      return NextResponse.json(
        { error: 'Edge functions not enabled for this site' },
        { status: 404 }
      );
    }
    ...
    const fn = siteDb.getFunctionByName(functionName);
    if (!fn) {
      return NextResponse.json(
    ...
    try {
      siteDb.logFunctionExecution(fn.id, {
        method: request.method,
